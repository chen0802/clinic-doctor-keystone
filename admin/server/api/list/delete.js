...
module.exports = function (req, res) {
	...
	var deletedCount = 0;
	var deletedIds = [];
	if ( req.list.onFilter != undefined ) {
		req.list.onFilter(req, function(err, filter){
			return deleteIds(filter)
		})
	} else {
		return deleteIds({})
	}
	function deleteIds (filter) {
		console.log(filter);
		req.list.model.find(filter).where('_id').in(ids).exec(function (err, results) {
			if (err) {
