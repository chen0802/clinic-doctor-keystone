...
import { SocketChannel } from '../../shared/React-Socket';
...
import ListHeaderToolbarEx from './components/ListHeaderToolbarEx';
...
const ListView = React.createClass({
	...
	onCreate (item) {
		...
		this.props.dispatch(setActiveSearch(this.props.active.search));
		window.displayModal(`${list.path}/${item.id}`);
		//.
		...
		return (
			<Container style={{ paddingTop: '2em' }}>
				...
				<ListHeaderToolbarEx
					...
					//// ADDON:OPTIONS
					optionsActive={this.props.active.options}
					optionsAvailable={this.props.currentList.options}

					//display components
					displayConfig = {this.props.currentList.display}

					//
					searchFieldsPlaceHolder = { this.props.currentList.searchFieldsPlaceHolder}
				/>
				<ListFilters
					dispatch={this.props.dispatch}
	...
	onMessage(message) {
        if ( message.type == 'update list' ) {
        	if ( message.list.indexOf( this.props.currentList.id ) >=0 || 
        		( message.list = "patient" && message.patient_name_is_modified )) 
        	{
        		// this.props.dispatch(selectList(this.props.params.listId));
        		// this.props.dispatch(setActiveSort(this.props.active.sort))
        		this.props.dispatch(setActiveSearch(this.props.active.search));
        	}
        }
    },
	render () {
		...
		return (
			<div data-screen-id="list">
				...
				/>
				<SocketChannel name={channel_name} handler={this.onMessage} />
